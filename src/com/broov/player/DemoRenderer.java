...
public class DemoRenderer extends GLSurfaceView_SDL.Renderer {
	...
	public DemoRenderer(Activity _context,Handler h){
		context = _context;
		this.mhandler = h;
	}
...
//		{
			if ((audioFileType == 1) && FileManager.isAudioStream(Globals.fileName)){
				...
				//Globals.fileName = "rtsp://59.40.0.239:28324/realplay?devid=004703578&channelno=0&streamtype=1&hashtoken=888888";
...
//				}
	...
	//视频打开加载结束回调函数
	public int exitLoadingScreen()
	{
		Message msg = mhandler.obtainMessage();
		msg.what = 1;
		mhandler.sendMessage(msg);
		System.out.println("Inside exitLoadingScreen()");
		return 1;
	}
	

	//播放器状态信息显示回调函数(stateRemark：状态描述)
	public void PlayerStateDisplay(String stateRemark)
	{
		Message msg = mhandler.obtainMessage();
		msg.what = 2;
		Bundle b = new Bundle();
        b.putString("stateRemark", stateRemark);
        msg.setData(b);
        mhandler.sendMessage(msg);
		System.out.println(stateRemark);
	}
    ...
    public native int  nativePlayerStartRecord(String strRecordPath);//本地录像JNI接口
    public native void nativePlayerStopRecord();//停止录像JNI接口
    public native int  nativePlayerIsRecord();//是否录像,0-没有录像,1-正在录像
    public native int  nativePlayerCaptureImage(String strImagePath);//本地抓拍JNI接口
    public native void nativePlayerOpenSound();//打开声音JNI接口
    public native void nativePlayerCloseSound();//关闭声音JNI接口
    public native int  nativePlayerIsSound();//是否监听,0-没有监听,1-正在监听
    public native int  nativePlayerReplay();//重新播放  
    public native int  nativePlayerIsPlaying();//是否正在播放, 0-没播, 1-正在播
